...
const ReviewList = ({ roomId }) => {
  ...
  return (
    <div className="grid md:grid-cols-2 p-3 gap-5 md:gap-10">
      {reviews?.map((review) => (
        <span key={review._id} className="max-w-[20rem] p-3 rounded-lg grid gap-2 border bg-zinc-100 shadow" >
        ...
        <span>
        <h4 className="text-[.74rem] font-[500] text-zinc-500 ">{review.senderName}</h4>

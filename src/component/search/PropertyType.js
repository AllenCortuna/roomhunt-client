  ...
  return (
    ...
    <span className="grid">
      <span className="grid grid-cols-3 gap-3 mt-2">
        {type.map((a) => (
          <span
            className={`group flex flex-wrap justify-center items-center text-center flex-row p-2 rounded-lg  transition-all duration-300 ease-linear shad ${query.category===a.name ? "bg-yellow " : "bg-smoke"}`}
            key={a.name}
            onClick={()=>handleCat(a.name)}
          >
            {a.icon}
            <p className={`truncate group-hover:text-gray-500 group-hover:font-semibold ml-1 inline text-[.8rem] group-hover:drop-shadow-md ${query.category === a.name ? "text-gray-500 font-semibold" : "text-gray-400"}`}>
              {a.name}
            </p>
          </span>
        ))}

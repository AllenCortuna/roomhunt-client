...
const PropertyType = ({ type }) => {
  return (
    <span className="grid mt-3 rounded-tl-md rounded-tr-md p-3 pb-4 bg-gray-100 shadow-sm mb-2">
      ...
      <span className="grid grid-cols-3 gap-2 mt-1">
        {type.map((a) => (
          <span className="group flex flex-wrap justify-center items-center text-center flex-row border border-gray-300 p-2 rounded-3xl hover:border-yellow hover:bg-yellow hover:shadow-md transition-all duration-300 ease-linear " key={a.name}>
            {a.icon}
